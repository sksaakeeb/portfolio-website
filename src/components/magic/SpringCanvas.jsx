  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      for (const node of nodes) {
        const dist = Math.hypot(
          node.pos.x - mouse.current.x,
          node.pos.y - mouse.current.y,
        );
    ...
    const animate = () => {
      ...
      for (const spring of springs) {
        ...
        const force = {
          x: (dir.x / dist) * forceMag,
          y: (dir.y / dist) * forceMag,
        };
        if (!a.pinned)
          a.acc = { x: a.acc.x + force.x / mass, y: a.acc.y + force.y / mass };
        if (!b.pinned)
          b.acc = { x: b.acc.x - force.x / mass, y: b.acc.y - force.y / mass };
      }
